package ru.tunkoff.fintech.qa.restassured.tests;

import io.restassured.RestAssured;
import io.restassured.filter.log.RequestLoggingFilter;
import io.restassured.filter.log.ResponseLoggingFilter;
import io.restassured.http.ContentType;
import ru.tunkoff.fintech.qa.models.pet.Category;
import ru.tunkoff.fintech.qa.models.pet.Pet;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

public class Put {

    @Test
    @DisplayName("New_200")
    public void putNew_200() {
        Pet pet = new Pet();
        pet.setId(1000311);
        pet.setName("Trixy1000311");
        Category category = new Category();
        category.setName("Cat");
        pet.setCategory(category);
        pet.setStatus("sold");
        int id = pet.getId();

        RestAssured.given()
                .contentType(ContentType.JSON)
                .pathParam("id", id)
                .delete("https://petstore.swagger.io/v2/pet/{id}");

        RestAssured.given()
                .contentType(ContentType.JSON)
                .filter(new RequestLoggingFilter())
                .filter(new ResponseLoggingFilter())
                .body(pet)
                .put("https://petstore.swagger.io/v2/pet").then()
                .statusCode(200);

        Pet updatedPet = RestAssured.given()
                .contentType(ContentType.JSON)
                .pathParam("id", id)
                .get("https://petstore.swagger.io/v2/pet/{id}").then()
                .statusCode(200)
                .extract().as(Pet.class);
        Assertions.assertEquals(pet, updatedPet);
    }

    @Test
    @DisplayName("Existing_200")
    public void putExisting_200() {
        Pet pet = new Pet();
        pet.setId(1000321);
        pet.setName("Trixy1000321");
        Category category = new Category();
        category.setName("Cat");
        pet.setCategory(category);
        pet.setStatus("available");
        int id = pet.getId();

        RestAssured.given()
                .contentType(ContentType.JSON)
                .pathParam("id", id)
                .delete("https://petstore.swagger.io/v2/pet/{id}");

        RestAssured.given()
                .contentType(ContentType.JSON)
                .body(pet)
                .post("https://petstore.swagger.io/v2/pet").then()
                .statusCode(200);

        pet.setStatus("sold");

        RestAssured.given()
                .contentType(ContentType.JSON)
                .filter(new RequestLoggingFilter())
                .filter(new ResponseLoggingFilter())
                .body(pet)
                .put("https://petstore.swagger.io/v2/pet").then()
                .statusCode(200);

        Pet updatedPet = RestAssured.given()
                .contentType(ContentType.JSON)
                .pathParam("id", id)
                .get("https://petstore.swagger.io/v2/pet/{id}").then()
                .statusCode(200)
                .extract().as(Pet.class);
        Assertions.assertEquals(pet, updatedPet);
    }

    @Test
    @DisplayName("404")
    public void put_404() {
        Pet pet = new Pet();
        pet.setId(1000331);
        pet.setName("Trixy1000331");
        Category category = new Category();
        category.setName("Cat");
        pet.setCategory(category);
        pet.setStatus("sold");
        int id = pet.getId();

        RestAssured.given()
                .contentType(ContentType.JSON)
                .pathParam("id", id)
                .delete("https://petstore.swagger.io/v2/wrong/{id}");

        RestAssured.given()
                .contentType(ContentType.JSON)
                .filter(new RequestLoggingFilter())
                .filter(new ResponseLoggingFilter())
                .body(pet)
                .put("https://petstore.swagger.io/v2/wrong/").then()
                .statusCode(404);
    }
}
